...
**参照 `templates/面试复盘模板.md` 生成** `interview/<session>.md`,结构:
0. 面试通过率(推测,百分制)—— 置于文档最前 + 评估依据
1. 总览 —— 评分汇总表 + 一句话结论
2. 问答实录 —— 逐题卡片:面试官问(详细)/ 候选人答(简略)/ 评分与点评(评分在前)
3. 面试缺点 —— 表达(结巴/犹豫/结构散集中在此)、紧张、答非所问、基础薄弱等
4. 下一步学习计划 —— 归并「可改进点 + 缺少知识点 + 下一步建议」,按重要性排序

逐题评分与整场通过率均用百分制;绝不根据文件名推断结果;逐题「候选人答」去口头禅、保留技术对错;通过率需附评估依据。

## 5. 更新知识点统计

把本场暴露的知识点累加进 `知识点统计.csv`(新知识点新增行、已有知识点频次 +1);排序时参考其顶部`weight` 注释。

## 6. 汇报

- 列出生成的文件:`resume/个人简历.md`、`interview/txt/<session>.txt`、`interview/<session>.md`、`知识点统计.csv`。
- 给出复盘关键结论,并提示可继续追问细节。
